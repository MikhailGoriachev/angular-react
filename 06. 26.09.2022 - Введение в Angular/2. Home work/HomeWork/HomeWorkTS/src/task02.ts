...
// •	имитация выезда автобуса из парка: задаётся номер автобуса; код удаляет данные об этом автобусе из
//      коллекции автобусов, находящихся в парке, и записывает эти данные в коллекцию автобусов, находящихся
//      на маршруте;
// •	имитация въезда автобуса в парк: задаётся номер автобуса; код удаляет данные об этом автобусе из
...
import {TransportCompany} from "./models/task02/TransportCompany";
import {BusState} from "./models/task02/BusState";
...
export class Task02 {
    ...
    // конструктор
    constructor(public output: { busTbody: JQuery, fleetTbody: JQuery, routeTbody: JQuery },
                public company: TransportCompany = new TransportCompany()) {
        company.init();
    }
    ...
    showBuses() {
        let n = 1, res = "";
        let isState = (p: string) => this.company.busFleet.has(p) ? BusState.busFleet : BusState.busRoute;
    ...
    showBusFleet() {
        ...
        for (const bus of this.company.generatorBusFleet())
            res += bus.toTableRow(n++, () => BusState.busFleet);
    ...
    showBusRoute() {
        ...
        for (const bus of this.company.generatorBusRoute())
            res += bus.toTableRow(n++, () => BusState.busRoute);
...
export function pageInitTask02() {
    $(() => {
        ...
        $("tbody").on("click", (e) => {
            ...
            let state: BusState = +target.data("state");
            let plate: string = target.data("plate");
